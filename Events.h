...
//#define MP_EVENT_ATTACH(eventName, eventPtr, methodSig) do { \
//		MonoMethod* m; \
//		if(!CMonoHelpers::GetMethod(this->m_image, methodSig, m, error, maxlen)) return false; \
//		eventPtr = new CEvent_##eventName(m); \
//		g_GameEventManager->AddListener(eventPtr, #eventName, true); \
//	} while(0);
