...
void BATCore::SetupPlayerInfo(int id)
{
	...
	if(!pEntity || !pSteamid)
	{
		g_BATCore.AddLogEntry("ERROR: All hell is loose, some client with no steamid is here, this normaly happens if a plugin does ServerExecte() server will crash now");
...
void BATCore::SendCornerMsg(int id,const char *fmt, ...)
{
	...
	va_list ap;
	va_start(ap, fmt);
	int len = Q_vsnprintf(vafmt, 239, fmt, ap);
	va_end(ap);
	len += Q_snprintf(&(vafmt[len]),239-len," \n");
...
void BATCore::ServerCommand(const char *fmt, ...)
{
	...
	va_list ap;
	va_start(ap, fmt);
	Q_vsnprintf(buffer, sizeof(buffer)-2, fmt, ap);
	va_end(ap);
	strcat(buffer,"\n");
...
void BATCore::MessagePlayer(int index, const char *msg, ...)
{
	 ...
	 va_list ap;
	 va_start(ap, msg);
	 int len = Q_vsnprintf(vafmt, 239, msg, ap);
	 va_end(ap);
...
void BATCore::AddLogEntry(const char * LogInfo, ...)
{
	...
	va_list ap;
	va_start(ap, LogInfo);
	Q_vsnprintf(vafmt,MAX_LOGLEN,LogInfo,ap);
	va_end(ap);
