...
extern bool g_ListenForChatCommands;
...
void BATVars::SetupCallBacks() // Yes i know we read all the cvars, when just 1 changed ( Or we can end up reading all 4 chars 4 times. But who cares, this should only happen on a mapchange and im lazy )
{
	...
	g_VarListenForChatCommands.InstallChangeCallback((FnChangeCallback_t) &BATVars::cbListenForChatCommands);
}
...
void BATVars::cbListenForChatCommands()
{
	g_ListenForChatCommands = g_VarListenForChatCommands.GetBool();
}
