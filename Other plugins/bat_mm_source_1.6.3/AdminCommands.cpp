...
void AdminCmd::CmdRcon(int admin_id)
{
	...
	if(g_ArgCount >= 2)
	{
		...
		{
			if(i == 1)
				len += Q_snprintf(&(SrvCmd[len]),191-len,"%s",GetArg(i));
			else
				len += Q_snprintf(&(SrvCmd[len]),191-len," %s",GetArg(i));
		}
	}
	else 
		Q_snprintf(SrvCmd,191,"%s",g_BATCore.StrRemoveQuotes(GetArg(1)));
...
void AdminCmd::CmdCSay(int admin_id)
{
	...
	if(g_ArgCount >= 2)
	{
		...
		{
			if(i == 1)
				len += Q_snprintf(&(SayMsg[len]),191-len,"%s",GetArg(i));
			else
				len += Q_snprintf(&(SayMsg[len]),191-len," %s",GetArg(i));
		}
	}
	else 
		len += Q_snprintf(&(SayMsg[len]),191-len,g_BATCore.StrRemoveQuotes(GetArg(1)));
...
void AdminCmd::CmdTest(int admin_id)
{
	...
	Q_snprintf(MyMsg,191,"\x01 This message is number 1");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x02 This message is number 2");
	g_BATCore.MessagePlayer(0,MyMsg);	

	Q_snprintf(MyMsg,191,"\x03 This message is number 3");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x04 This message is number 4");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x05 This message is number 5");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x06 This message is number 6");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x07 This message is number 7");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x08 This message is number 8");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x09 This message is number 9");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x11 This message is number 11");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x12 This message is number 12");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x13 This message is number 13");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x14 This message is number 14");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x15 This message is number 15");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x16 This message is number 16");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x17 This message is number 17");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x18 This message is number 18");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x19 This message is number 19");
	g_BATCore.MessagePlayer(0,MyMsg);

	Q_snprintf(MyMsg,191,"\x20 This message is number 20");
	g_BATCore.MessagePlayer(0,MyMsg);
	*/
}
#endif
...
void AdminCmd::AdminTalkToAll(int admin_id,const char *pText)
{
	...
	Q_snprintf(buffer,191,"%s - %s",g_BATCore.GetPlayerName(admin_id),pText);
...
void AdminCmd::ChangeTeam(int target_id,int admin_id,int Team)
{
	...
	/*
	char ClientCmd[24];
	Q_snprintf(ClientCmd,sizeof(ClientCmd)-1,"jointeam %d",Team);
...
void AdminCmd::SetNextmap(int admin_id,char *Nextmap)
{
	...
	Q_snprintf(g_NextMapName,MAX_MAPNAMELEN,"%s",Nextmap);
...
{
	char MyMsg[192];
	Q_snprintf(MyMsg,191,"\x03 ADMIN Chat - %s\x01: %s",g_BATCore.GetPlayerName(id),temp);
...
void AdminCmd::NoticePlayer(int admin_id, const char *msg, ...)
{
	...
	va_list ap;
	va_start(ap, msg);
	int len = Q_vsnprintf(vafmt, 191, msg, ap);
	va_end(ap);
...
char *AdminCmd::GetReason(unsigned int ArgStart)
{
	...
	{
		if(i == 1)
			len += Q_snprintf(&(Reason[len]),191-len,"%s",GetArg(i));
		else
			len += Q_snprintf(&(Reason[len]),191-len," %s",GetArg(i));
	}
	return Reason;
