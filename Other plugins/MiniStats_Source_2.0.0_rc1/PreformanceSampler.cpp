...
{
	long ThingsProfiled = 0;
	for(int i=0;i<MAX_SAMPLEITEMS;i++)
		 ThingsProfiled += g_SampleList[i].CalledTimes;
	...
	{
		if(g_SampleList[i].CalledTimes == 0)
			continue;
	...
	for(int i=0;i<MAX_SAMPLEITEMS;i++)
	{
		g_SampleList[i].TotalTime = 0;
		g_SampleList[i].CalledTimes = 0;
		g_SampleList[i].ShortestTime = ULONG_MAX;
		g_SampleList[i].LongestTime = 0;
	}
}
const char *PreformanceSampler::GetTimeInformationString(__int64 Time)
{
	...
	if(Seconds != 0)
	{
		...
		_snprintf(sTempString,sizeof(sTempString),"%lld microseconds | %lld milliseconds | %ll seconds",Time,MiliSeconds,Seconds);
	}
	else if(MiliSeconds != 0)
	{
		double dSeconds = GetSecondsWithComa(Time);
		_snprintf(sTempString,sizeof(sTempString),"%lld microseconds | %lld milliseconds | %f seconds",Time,MiliSeconds,dSeconds);
	}
	else
	{
		double dSeconds = GetSecondsWithComa(Time);
		_snprintf(sTempString,sizeof(sTempString),"%lld microsecond | %f seconds",Time,dSeconds);
	}
...
double PreformanceSampler::GetSecondsWithComa(__int64 Time)
{
	double dTime = (double)Time;
	return dTime / 1000000;
}
