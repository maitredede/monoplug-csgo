...
namespace ts3_server_minimal_sample
{
	...
	class ts3server {
		...
		*/
		public static int readKeyPairFromFile(string fileName, out string keyPair) {
			try {
				...
				Console.WriteLine("Could not open file '{0}' for reading keypair\n", fileName);
				keyPair = "";
				return -1;
			}
		...
		*/
		public static int writeKeyPairToFile(string fileName, string keyPair) {
			...
			public static extern uint ts3server_createVirtualServer(int serverPort, string ip, string serverName, string serverKeyPair, uint serverMaxClients, out anyID serverID);
			...
			public static extern uint ts3server_getVirtualServerKeyPair(anyID serverID, out IntPtr result);

			[DllImport("ts3server_win64.dll", EntryPoint = "ts3server_setVirtualServerVariableAsString")]
			public static extern uint ts3server_setVirtualServerVariableAsString(anyID serverID, VirtualServerProperties flag, string result);

			[DllImport("ts3server_win64.dll", EntryPoint = "ts3server_flushVirtualServerVariable")]
			public static extern uint ts3server_flushVirtualServerVariable(anyID serverID);

			[DllImport("ts3server_win64.dll", EntryPoint = "ts3server_stopVirtualServer")]
			public static extern uint ts3server_stopVirtualServer(anyID serverID);
		#else
	...
	class Program {
		static void Main(string[] args) {
            ...
            cbs.onTextMessageEvent_delegate = new onTextMessageEvent_type(callback.onTextMessageEvent);
			...
			ts3server.ts3server_initServerLib(ref cbs, LogTypes.LogType_FILE | LogTypes.LogType_CONSOLE | LogTypes.LogType_USERLOGGING, null);

			/* Query and print client lib version */
			IntPtr versionPtr = IntPtr.Zero;
			uint error = ts3server.ts3server_getServerLibVersion(out versionPtr);
			...
			if (error != public_errors.ERROR_ok) {
				...
				if (error == public_errors.ERROR_ok) {
					...
					Console.WriteLine("Error creating virtual server: {0} ({1})\n", errormsg, error);
