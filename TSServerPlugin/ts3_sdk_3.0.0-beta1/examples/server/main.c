...
void onClientConnected(anyID serverID, anyID clientID, anyID channelID, unsigned int* removeClientError) {
    ...
    printf("Client '%s' joined channel %u on virtual server %u\n", clientName, channelID, serverID);
 ...
 *   targetIDS       - 0-terminated array of the target IDs. In case of a channel text message the array will be
 *                     { <channelID>, 0 }, in case of a server chat the array will be { 0 }.
...
void onTextMessageEvent(anyID serverID, enum TextMessageTargetMode textMessageMode, anyID invokerClientID, const anyID* targetIDS, const char* textMessage) {
	...
	/* Note: Private chats are not received */
	switch(textMessageMode) {
		case TextMessageTarget_CHANNEL:
		{
			/* Get channel name */
			char* channelName;
			if((error = ts3server_getChannelVariableAsString(serverID, targetIDS[0], CHANNEL_NAME, &channelName)) != ERROR_ok) {
				printf("Error getting channel name: %d\n", error);
				return;
			}
			printf("Text message in channel '%s' by %s: %s\n", channelName, invokerNickname, textMessage);
			ts3server_freeMemory(channelName);  /* Release previously allocated memory */
			break;
		}
...
void showVirtualServers() {
	...
	for(i=0; ids[i]; i++) {
		...
		printf("ID=%u  NAME=\"%s\"  CAPACITY=%d  Unique Identifier=\"%s\"\n", ids[i], name, slotCount, virtualServerUniqueIdentifier);
...
void showChannels(anyID serverID) {
	anyID* ids;
	int i;
	unsigned int error;
...
void showClients(anyID serverID) {
	anyID* ids;
	int i;
	unsigned int error;

	printf("\nList of clients on virtual server %d:\n", serverID);
	...
	for(i=0; ids[i]; i++) {
        ...
        if((error = ts3server_getClientVariableAsInt(serverID, ids[i], CLIENT_FLAG_TALKING, &talkStatus)) != ERROR_ok) {  /* and talkstatus */
            printf("Error querying client nickname: %d\n", error);
            break;
        }
		printf("%u - %s (%stalking)\n", ids[i], name, (talkStatus ? "" : "not "));
		ts3server_freeMemory(name);  /* Do not free memory if above function returned an error */
	}
	printf("\n");
...
void createChannel(anyID serverID, const char *name) {
	unsigned int error;
	anyID newChannelID;
...
void deleteChannel(anyID serverID) {
...
void renameChannel(anyID serverID) {
	unsigned short channelID;
	int n;
	unsigned int error;
	char name[BUFSIZ];
...
void moveClient(anyID serverID) {
	anyID clientIDArray[2];  /* We only want to move one client plus terminating null end-marker */
	anyID newChannelID;  /* ID of channel to move the client into */
	unsigned int error;
	int n;
	...
	printf("Client %d moved to channel %d\n", clientIDArray[0], newChannelID);
}

anyID createVirtualServer(const char* name, int port, unsigned int maxClients) {
