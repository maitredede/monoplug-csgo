 ...
 */
void onTextMessageEvent(uint64 serverID, enum TextMessageTargetMode textMessageMode, anyID invokerClientID, const char* textMessage) {
	...
	switch(textMessageMode) {
		...
		{
			uint64 channelOfInvokerClient;
			char* channelName;

			/* Get the channel ID of the invoking client (client can only send chats in his current channel) */
			if((error = ts3server_getChannelOfClient(serverID, invokerClientID, &channelOfInvokerClient)) != ERROR_ok) {
				printf("Error getting channel of invoker client: %d\n", error);
				return;
			}

			/* Get channel name */
			if((error = ts3server_getChannelVariableAsString(serverID, channelOfInvokerClient, CHANNEL_NAME, &channelName)) != ERROR_ok) {
