...
[UnmanagedFunctionPointer(CallingConvention.Cdecl)]
public delegate void onTextMessageEvent_type(uint64 serverID, int textMessageMode, anyID invokerClientID, string textMessage);
[UnmanagedFunctionPointer(CallingConvention.Cdecl)]
...
public static class callback {
     ...
     */
    public static void onTextMessageEvent(uint64 serverID, int textMessageMode, anyID invokerClientID, string textMessage)
    {
        // Note: Client targets are not received
        if(textMessageMode == 2) {  // Channel
            Console.WriteLine("Text message in channel chat by {0}: {1}", invokerClientID, textMessage);
