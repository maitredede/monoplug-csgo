...
namespace MonoPlug
{
    partial class ClsMain
    {
        ...
        private static extern void Attach_ClientCommand();
        [MethodImpl(MethodImplOptions.InternalCall)]
        private static extern void Detach_ClientCommand();

        private static readonly object _evtClientCommand = new object();

        internal void ClientCommand_Add(ClsPluginBase plugin)
        {
            this.Event_Add(plugin, _evtClientCommand, Attach_ClientCommand);
        }

        internal void ClientCommand_Remove(ClsPluginBase plugin)
        {
            this.Event_Remove(plugin, _evtClientCommand, Detach_ClientCommand);
        }
