...
namespace MonoPlug
{
    public sealed class ClsInstallTools : ClsPluginBase
    {
...
#endif
            this._test_mysql = this.ConItem.RegisterConCommand("clr_mysql_test", "Test MySQL connection", FCVAR.FCVAR_NONE, this.MySQL_Test, null, true);
#if DEBUG
            this.Message.DevMsg("{0}: ({1} : {2})\n", this.Name, "loading", "B");
#endif
            this._geoip_install = this.ConItem.RegisterConCommand("clr_geoip_install", "Create GeoIP table and download data", FCVAR.FCVAR_NONE, this.InstallGeoIP, null, true);
#if DEBUG
            this.Message.DevMsg("{0}: ({1} : {2})\n", this.Name, "loading", "X");
#endif
        }
        ...
        {
            this.ConItem.UnregisterConCommand(this._test_mysql);
        }
