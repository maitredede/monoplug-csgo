...
{
	bool CMonoHelpers::GetDomain(const char* file, const char* rootDir, MonoDomain*& domain, char *error, size_t maxlen)
	{
	...
	bool CMonoHelpers::GetAssembly(const char* file, MonoDomain* domain, MonoAssembly*& assembly, char *error, size_t maxlen)
	{
	...
	bool CMonoHelpers::GetImage(const char* file, MonoAssembly* assembly, MonoImage*& image, char *error, size_t maxlen)
	{
	...
	bool CMonoHelpers::GetClass(MonoImage *assemblyImage, const char *classNamespace, const char *className, MonoClass*& classPtr, char *error, size_t maxlen)
	{
	...
	bool CMonoHelpers::GetField(MonoClass* classPtr, MonoClassField*& fieldPtr, const char* fieldName, char *error, size_t maxlen)
	{
		fieldPtr = mono_class_get_field_from_name(classPtr, fieldName);
		if(!fieldPtr)
		{
			g_SMAPI->Format(error, maxlen, "Can't get field %s", fieldName);
			return false;
		}
		return true;
	}

	MonoString* CMonoHelpers::GetString(MonoDomain* domain, const char* string)
	{
		if(string == NULL)
		{
			return NULL;
	...
	MonoObject* CMonoHelpers::CallDelegate(MonoDelegate *delegateObject, void **args)
	{
	...
	MonoObject* CMonoHelpers::CallMethod(void *target, MonoMethod *methodHandle, void **args)
	{
	...
	MonoObject* CMonoHelpers::ClassNew(MonoDomain* domain, MonoClass *cls)
	{
	...
	bool CMonoHelpers::GetMethod(MonoImage* assemblyImage, const char* methodSig, MonoMethod*& methodptr, char *error, size_t maxlen)
	{
